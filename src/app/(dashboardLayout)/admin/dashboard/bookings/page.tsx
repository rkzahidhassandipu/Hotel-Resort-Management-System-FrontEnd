...
import BookingDetailSlideOver from "@/components/booking/BookingDetailSlideOver";
...
interface BookingListResponse {
  data?: {
    data?: Booking[] | { data: Booking[]; total: number };
    total?: number;
  };
}
...
export default function AdminBookingsPage() {
  ...
  mutationFn: ({ action, id }: { action: string; id: string }) => {
    ...
    return bookingService.cancel(id, 'Cancelled by admin');
  },
  onMutate: ({ action, id }) => setPendingKey(id + action),
  onSuccess: (_, { action }) => {
    if (action === 'confirm')  toast.success('Booking confirmed');
    if (action === 'checkin')  toast.success('Guest checked in');
    if (action === 'checkout') toast.success('Guest checked out');
    if (action === 'cancel')   toast.success('Booking cancelled & refund initiated');
  },
  onError: (_, { action }) => {
      ...
      render: (_, r) => {
        ...
        return (
          <div className="flex items-center gap-1">
            {r.status === "PENDING" && (
              <button
                onClick={() => doAction({ action: "confirm", id: r.id })}
                disabled={busy}
                className="p-1.5 rounded text-[#37EFD1] hover:bg-[#37EFD1]/10 transition-colors disabled:opacity-50"
                title="Confirm"
              >
                {spin("confirm") ? (
            ...
            {r.status === "CONFIRMED" && (
              <button
                onClick={() => doAction({ action: "checkin", id: r.id })}
                disabled={busy}
                className="p-1.5 rounded text-[#60a5fa] hover:bg-[#60a5fa]/10 transition-colors disabled:opacity-50"
                title="Check In"
              >
                {spin("checkin") ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ...
            {r.status === "CHECKED_IN" && (
              <button
                onClick={() => doAction({ action: "checkout", id: r.id })}
                disabled={busy}
                className="p-1.5 rounded text-[#fb923c] hover:bg-[#fb923c]/10 transition-colors disabled:opacity-50"
                title="Check Out"
              >
                {spin("checkout") ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ...
            {!["CHECKED_IN", "CHECKED_OUT", "CANCELLED"].includes(r.status) && (
              <button
                onClick={() => doAction({ action: "cancel", id: r.id })}
                disabled={busy}
