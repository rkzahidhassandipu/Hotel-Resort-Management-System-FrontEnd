...
export const authService = {
  ...
  login2fa: (data: { email: string; token: string }) => httpClient.post('/auth/login/2fa', data),
  ...
  verifyEmail: (token: string) => httpClient.post('/auth/verify-email', { token }),
  resendVerifyEmail: (email: string) => httpClient.post('/auth/resend-verify-email', { email }),
  me: () => httpClient.get('/auth/me'),
  changePassword: (data: { currentPassword: string; newPassword: string }) => httpClient.put('/auth/change-password', data),
  refreshToken: (refreshToken: string) => httpClient.post('/auth/refresh-token', { refreshToken }),
