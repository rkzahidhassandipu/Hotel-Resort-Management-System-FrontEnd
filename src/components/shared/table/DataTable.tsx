      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-white/5 bg-[#0B0C10]">
            {columns.map(col => (
              <th key={col.key} className={`text-left px-4 py-3 text-[10px] font-sans uppercase tracking-widest text-white/30 font-normal ${col.className || ''}`}>
                {col.sortable ? (
          ...
          {loading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} className="border-b border-white/3">
                {columns.map(col => (
                  <td key={col.key} className="px-4 py-3"><div className="h-4 rounded skeleton w-24" /></td>
                ))}
              </tr>
            ))
          ) : data.length === 0 ? (
            ...
            data.map((row, i) => (
              <tr key={i} className="border-b border-white/3 last:border-0 hover:bg-white/2 transition-colors">
                {columns.map(col => (
                  <td key={col.key} className={`px-4 py-3 text-sm font-sans text-white/70 ${col.className || ''}`}>
