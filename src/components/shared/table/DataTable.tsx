        ...
        <thead>
          <tr className="border-b border-white/5 bg-[#0B0C10]">
            {columns.map((col, colIndex) => (
              <th key={`${col.key}-${colIndex}`} className={`text-left px-4 py-3 text-[10px] font-sans uppercase tracking-widest text-white/30 font-normal ${col.className || ''}`}>
                {col.sortable ? (
          ...
          {loading ? (
            Array.from({ length: 5 }).map((_, rowIndex) => (
              <tr key={rowIndex} className="border-b border-white/3">
                {columns.map((col, colIndex) => (
                  <td key={`${col.key}-${rowIndex}-${colIndex}`} className="px-4 py-3">
            ...
            <tr>
              <td colSpan={columns.length} className="px-4 py-12 text-center text-white/25 text-sm font-sans">
                {emptyMessage}
              </td>
            </tr>
          ) : (
