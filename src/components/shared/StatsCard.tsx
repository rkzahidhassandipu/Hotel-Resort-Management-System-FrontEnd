    ...
    <div className="bg-[#1A1B21] border border-white/5 rounded-xl p-5 relative overflow-hidden group hover:border-white/10 transition-all">
      ...
      <div className="flex items-start justify-between">
        <div className="min-w-0 flex-1">
          ...
          {/* title */}
          <p className="text-white/35 text-[10px] font-sans uppercase tracking-[0.15em] mb-2">
            {title}
          </p>
          ...
          {/* subtitle */}
          {subtitle && !loading && (
            <p className="text-white/30 text-xs font-sans mt-1 truncate">
              {subtitle}
            </p>
          )}
          ...
          {trend && !loading && (
            <div
              className={`flex items-center gap-1 mt-2 text-xs font-sans ${
                trend.value > 0 ? 'text-[#37EFD1]' : 'text-[#C8102E]'
              }`}
            >
              {trend.value > 0 ? (
