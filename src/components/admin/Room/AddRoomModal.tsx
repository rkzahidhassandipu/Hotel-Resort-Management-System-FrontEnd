...
import { Amenity, CreateRoomPayload, RoomCategory } from "@/types";
...
export default function AddRoomModal({
  ...
  // ✅ Fix: robust data extraction for both intercepted and raw axios responses
  const categories: RoomCategory[] = (() => {
    ...
    // Filter to ensure only valid category objects with expected properties
    return items.filter((item) => {
      ...
      return (
        "id" in item &&
        "name" in item &&
        ("basePrice" in item || "base_price" in item)
      );
    });
  })();

  const amenities: Amenity[] = (() => {
    const raw = amenRes?.data;
    let items: any[] = [];
    ...
    // Filter to ensure only valid amenity objects with expected properties
    return items.filter((item) => {
      if (!item || typeof item !== "object" || Array.isArray(item))
        return false;
      // Must have these core properties
  ...
  const { mutate: createRoom, isPending } = useMutation<unknown, unknown, CreateRoomPayload>({
    mutationFn: async (payload) => {
      const amenityIds = payload.amenityIds;
      const createPayload = { ...payload } as Record<string, unknown>;
      delete createPayload.amenityIds;

      const createResult = await roomService.create(createPayload as unknown as CreateRoomPayload);
      const roomId = createResult.data?.data?.id ?? createResult.data?.id;

      if (amenityIds?.length && roomId) {
        await roomService.updateAmenities(String(roomId), amenityIds);
      }
    ...
    onSuccess: () => {
      setToast({
        type: "success",
        msg: `Room #${form.roomNumber} created successfully`,
      });
    ...
    onError: (err: any) => {
      ...
      if (typeof errorData?.message === "string") {
        ...
        errorMsg = errorData.message.name || JSON.stringify(errorData.message);
  ...
  function validate() {
    ...
    if (form.maxOccupancy && isNaN(Number(form.maxOccupancy)))
      e.maxOccupancy = "Must be a number";
    if (form.sizeInSqFt && isNaN(Number(form.sizeInSqFt)))
      e.sizeInSqFt = "Must be a number";
    setErrors(e);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const payload: CreateRoomPayload = {
      ...
      type: form.type as CreateRoomPayload["type"],
      bedType: form.bedType as CreateRoomPayload["bedType"],
    ...
    console.debug("Create room payload:", payload);
    createRoom(payload);
  }
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => set("sizeInSqFt", e.target.value)}
              />
                      ...
                      {items.map((a) => {
                        ...
                        return (
                          ...
                          >
                            {selected && (
                              <Check
                                className="inline h-2.5 w-2.5 mr-1"
                                strokeWidth={3}
                              />
                            )}
                            {a.name}
